...
void setPLIST( u_int32_t ip, u_int32_t sport, u_int32_t dport, int len ){
  ...
  while(1){
    ...
    if( cell->next==NULL || cell->next->ip > ip ){
      ...
      cell_new->ip = ip; 
      cell_new->sport = sport; 
      cell_new->dport = dport; 
      cell_new->len = len;
...
void printPLIST(){
  ...
  while(1){
    ...
    while(1){
      ...
      for( i=0; i<(double)cell->len/100.0; i++ ) printf("■");
      printf("\n");
...
int f;
#define CREATE_IPADR(a,b,c,d) ( a<<0 | b<<8 | c<<16 | d<<24 )
void get_packet( u_char *arg, const struct pcap_pkthdr *h, const u_char *p ){
  ...
  if( ip->ip_dst.s_addr != CREATE_IPADR(160,16,74,85) ) return;
...
int main( void ){
  ...
  // デバイスオープン
  if( (handle = pcap_open_live( dev, DPCP_RCV_MAXSIZE, DPCP_PROMSCS_MODE, DPCP_RCV_TIMEOUT, ebuf )) == NULL ){
    printf("デバイスが開けません、実行許可を確認してください。\n");
    return -1;
  }
  ...
  pthread_t thread;
  pthread_create( &thread, NULL, (void *)print_thread, (void *)handle );
